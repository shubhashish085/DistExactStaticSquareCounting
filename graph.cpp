...
void Graph::loadPartitionedLocalGraphWithOnlyCutEdgesFromFile(const std::string& file_path, const std::string& vertex_partition_file_path, int partition_no)
{
    VertexID vertex_id, begin, end;
    NodeID partition_id;
    bool contains_in_partition = false;
...
void Graph::loadCutGraphWithLocalCutEdges(const std::string& file_path, const std::string& vtx_ptn_file_path, NodeID partition_idx,
                             std::vector<std::pair<VertexID, VertexID>>& local_cut_edges)
{
...
void Graph::loadCutGraphWithLocalCutEdgesOptimized(const std::string& file_path, const std::string& vtx_ptn_file_path, NodeID partition_idx,
                             std::vector<std::pair<VertexID, VertexID>>& local_cut_edges)
{
...
bool Graph::is_smaller_core_only(VertexID u, VertexID v){
...
void Graph::transformToAugmentedGraphWithOnlyCoreOrdering(Graph* augmented_graph){
    ...
    for (ui i = 0; i < vertices_count; i++)
    {
        for (ui j = offsets[i]; j < offsets[i + 1]; j++)
        {
            if ((augmented_graph->core[neighbors[j]] > augmented_graph->core[i]) || (augmented_graph->core[neighbors[j]] == augmented_graph->core[i]) && (neighbors[j] > i))
            {
                augmented_graph->degrees[i] += 1;
            }
        }
