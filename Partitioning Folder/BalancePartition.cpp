...
int main(int argc, char** argv){
    ...
    NodeID* part = new NodeID[graph->vertices_count];
    ...
    for (int i = 0; i < 8; i++){
        ...
        for(ui j = 0; j < graph->vertices_count; j++){
            part[j] = -1;
        }

        //partitionWithDegreeAndCut(graph, nParts, part, num_iterations);
        partitionWithDegreeAndCutForInterations(graph, nParts, part, num_iterations);
          
        std::string final_output_file =  std::to_string(nParts) + "_i" + std::to_string(num_iterations) + "_" +  dataset_name;

        write_partitioned_vertices(graph->vertices_count, part, final_output_file);
