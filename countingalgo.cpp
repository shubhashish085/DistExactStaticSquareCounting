...
long long CountingAlgorithm::local_count_square_in_four_partitions(Graph* graph, std::vector<std::pair<VertexID, VertexID>> local_cut_edge_list){
...
long long CountingAlgorithm::sequential_db_count_square_with_core_ordering(Graph* graph){
    ...
    for (VertexID v1 = 0; v1 < graph->vertices_count; v1++)
    {
        ...
        for (VertexID j = 0; j < nbrs_1_cnt; j++)
        {
            ...
            for (VertexID k = 0; k < nbrs_1_cnt; k++)
            {
                count_1 = 0;
                v3 = nbrs_1[k];
                if(!(graph->is_smaller_core(v2, v3))){
                    continue;
                }
...
void CountingAlgorithm::db_count_square_with_cut_graph_parallel(const std::string& file_path, const std::string& vertex_partition_file_path, int partition_cnt){
...
void CountingAlgorithm::db_count_square_with_cut_graph_parallel_optimized(const std::string& file_path, const std::string& vertex_partition_file_path, int partition_cnt){
